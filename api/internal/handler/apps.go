// api/internal/handler/apps.go
package handler

import (
	"fmt"
	"strings"

	"github.com/gofiber/fiber/v2"
	"go.temporal.io/sdk/client"
	"go.uber.org/zap"

	"github.com/tu-usuario/hangar/api/internal/db"
	"github.com/tu-usuario/hangar/api/internal/models"
	"github.com/tu-usuario/hangar/api/internal/workflows"
)

// AppHandler agrupa los handlers relacionados a apps
type AppHandler struct {
	apps     *db.AppRepository
	deploys  *db.DeploymentRepository
	temporal client.Client
	log      *zap.Logger
}

// NewAppHandler crea un nuevo handler de apps
func NewAppHandler(
	apps *db.AppRepository,
	deploys *db.DeploymentRepository,
	temporal client.Client,
	log *zap.Logger,
) *AppHandler {
	return &AppHandler{
		apps:     apps,
		deploys:  deploys,
		temporal: temporal,
		log:      log,
	}
}

// Register registra todas las rutas de apps en el router
func (h *AppHandler) Register(router fiber.Router) {
	router.Get("/apps", h.ListApps)
	router.Post("/apps", h.CreateApp)
	router.Get("/apps/:id", h.GetApp)
	router.Post("/apps/:id/deploy", h.TriggerDeploy)
	router.Get("/apps/:id/deployments", h.ListDeployments)
	router.Get("/deployments/:id/logs", h.GetDeployLogs)
}

// ListApps GET /api/v1/apps
func (h *AppHandler) ListApps(c *fiber.Ctx) error {
	apps, err := h.apps.List(c.Context())
	if err != nil {
		h.log.Error("error listando apps", zap.Error(err))
		return c.Status(500).JSON(fiber.Map{"error": "error interno"})
	}

	return c.JSON(fiber.Map{
		"apps":  apps,
		"total": len(apps),
	})
}

// CreateApp POST /api/v1/apps
func (h *AppHandler) CreateApp(c *fiber.Ctx) error {
	var req models.CreateAppRequest
	if err := c.BodyParser(&req); err != nil {
		return c.Status(400).JSON(fiber.Map{"error": "body inválido"})
	}

	if req.Name == "" || req.GitURL == "" {
		return c.Status(400).JSON(fiber.Map{
			"error": "name y git_url son requeridos",
		})
	}

	subdomain := generateSubdomain(req.Name)

	exists, err := h.apps.SubdomainExists(c.Context(), subdomain)
	if err != nil {
		h.log.Error("error verificando subdominio", zap.Error(err))
		return c.Status(500).JSON(fiber.Map{"error": "error interno"})
	}
	if exists {
		subdomain = fmt.Sprintf("%s-%s", subdomain, randomSuffix(4))
	}

	app, err := h.apps.Create(c.Context(), req.Name, req.GitURL, subdomain)
	if err != nil {
		h.log.Error("error creando app", zap.Error(err))
		return c.Status(500).JSON(fiber.Map{"error": "error creando app"})
	}

	h.log.Info("app creada",
		zap.String("id", app.ID),
		zap.String("subdomain", app.Subdomain),
	)

	return c.Status(201).JSON(models.CreateAppResponse{
		App:     *app,
		Message: fmt.Sprintf("App lista. Despliega con POST /api/v1/apps/%s/deploy", app.ID),
	})
}

// GetApp GET /api/v1/apps/:id
func (h *AppHandler) GetApp(c *fiber.Ctx) error {
	id := c.Params("id")

	app, err := h.apps.GetByID(c.Context(), id)
	if err != nil {
		return c.Status(404).JSON(fiber.Map{"error": "app no encontrada"})
	}

	return c.JSON(app)
}

// TriggerDeploy POST /api/v1/apps/:id/deploy
// Ahora dispara un workflow real de Temporal en lugar de solo crear el registro.
func (h *AppHandler) TriggerDeploy(c *fiber.Ctx) error {
	id := c.Params("id")

	// Verifica que la app existe
	app, err := h.apps.GetByID(c.Context(), id)
	if err != nil {
		return c.Status(404).JSON(fiber.Map{"error": "app no encontrada"})
	}

	var req models.TriggerDeployRequest
	c.BodyParser(&req) // opcional

	// Crea el registro del deployment en DB
	deployment, err := h.deploys.Create(c.Context(), app.ID, req.CommitSha)
	if err != nil {
		h.log.Error("error creando deployment", zap.Error(err))
		return c.Status(500).JSON(fiber.Map{"error": "error iniciando deploy"})
	}

	// Arma el input del workflow
	input := workflows.DeployInput{
		DeploymentID: deployment.ID,
		AppID:        app.ID,
		AppName:      app.Name,
		GitURL:       app.GitURL,
		Subdomain:    app.Subdomain,
		CommitSha:    req.CommitSha,
	}

	// El WorkflowID es determinista: una app solo puede tener
	// un deploy activo a la vez con este ID.
	workflowID := fmt.Sprintf("deploy-%s-%s", app.ID, deployment.ID)

	// Dispara el workflow en Temporal
	// StartWorkflow es no-bloqueante - devuelve inmediatamente
	// y el workflow corre en el worker en background
	we, err := h.temporal.ExecuteWorkflow(
		c.Context(),
		client.StartWorkflowOptions{
			ID:        workflowID,
			TaskQueue: "hangar-deploy",
		},
		workflows.DeployWorkflow,
		input,
	)
	if err != nil {
		h.log.Error("error disparando workflow",
			zap.String("app_id", app.ID),
			zap.Error(err),
		)
		// Marca el deployment como fallido si Temporal no responde
		_ = h.deploys.UpdateStatus(c.Context(), deployment.ID, models.DeploymentStatusFailed)
		return c.Status(500).JSON(fiber.Map{"error": "error iniciando workflow de deploy"})
	}

	h.log.Info("workflow iniciado",
		zap.String("workflow_id", we.GetID()),
		zap.String("run_id", we.GetRunID()),
		zap.String("app", app.Name),
	)

	return c.Status(202).JSON(fiber.Map{
		"deployment":  deployment,
		"workflow_id": we.GetID(),
		"run_id":      we.GetRunID(),
		"message":     fmt.Sprintf("Deploy iniciado. Sigue el progreso en http://localhost:8088"),
	})
}

// ListDeployments GET /api/v1/apps/:id/deployments
func (h *AppHandler) ListDeployments(c *fiber.Ctx) error {
	id := c.Params("id")

	if _, err := h.apps.GetByID(c.Context(), id); err != nil {
		return c.Status(404).JSON(fiber.Map{"error": "app no encontrada"})
	}

	deployments, err := h.deploys.ListByApp(c.Context(), id)
	if err != nil {
		h.log.Error("error listando deployments", zap.Error(err))
		return c.Status(500).JSON(fiber.Map{"error": "error interno"})
	}

	return c.JSON(fiber.Map{
		"deployments": deployments,
		"total":       len(deployments),
	})
}

// GetDeployLogs GET /api/v1/deployments/:id/logs
func (h *AppHandler) GetDeployLogs(c *fiber.Ctx) error {
	id := c.Params("id")

	logs, err := h.deploys.GetLogs(c.Context(), id)
	if err != nil {
		return c.Status(404).JSON(fiber.Map{"error": "deployment no encontrado"})
	}

	return c.JSON(fiber.Map{
		"logs":  logs,
		"total": len(logs),
	})
}

// -- Helpers --------------------------------------------------------------─

func generateSubdomain(name string) string {
	s := strings.ToLower(name)
	s = strings.ReplaceAll(s, " ", "-")
	var result strings.Builder
	for _, r := range s {
		if (r >= 'a' && r <= 'z') || (r >= '0' && r <= '9') || r == '-' {
			result.WriteRune(r)
		}
	}
	return strings.Trim(result.String(), "-")
}

func randomSuffix(n int) string {
	const chars = "abcdefghijklmnopqrstuvwxyz0123456789"
	b := make([]byte, n)
	for i := range b {
		b[i] = chars[i%len(chars)]
	}
	return string(b)
}
